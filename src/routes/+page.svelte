<script>
	...
	let actions = ['start up', 'stand out', 'stand strong', 'start now'];
	...
	setInterval(() => {
		...
		// action = actions[randomEl(actions)];
	}, 5000);
</script>
	...
	<div class="mx-auto max-w-[85rem] px-4 py-20 sm:px-6 lg:px-8">
		<div class="relative mx-auto grid max-w-4xl space-y-5 sm:space-y-10">
			...
			<div class="text-center">
				<p class="mb-3 text-xs font-semibold tracking-wide text-muted-foreground">Tandika</p>
				<h1 class="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">
					Helping African Startups
					<br />
					{#key index}
						<span in:fade class="text-5xl text-[#69306D]">
							{actions[index]}
						</span>
					{/key}
				</h1>
			...
			<div class="text-center sm:flex sm:items-center sm:justify-center sm:text-start">
				<div class="flex-shrink-0 pb-5 sm:flex sm:pb-0 sm:pe-5">
					...
					<div class="flex justify-center -space-x-3">
						<Avatar.Root class="size-10 border bg-background shadow-sm">
							<Avatar.Image src="/bulb.png" alt="startup" />
						...
						<span
							class="z-10 inline-flex size-10 items-center justify-center rounded-full bg-background ring-2 ring-muted-foreground"
						>
							<span class="text-xs font-medium uppercase leading-none"> 10+ </span>
						</span>
					</div>
					<!-- {/* End Avatar Group */} -->
				</div>
			...
			<form class="flex flex-col gap-2">
				<div
					class="mx-auto h-px w-32 border-t sm:mx-0 sm:h-full sm:w-auto sm:border-s sm:border-t-0"
				/>
				<div class="pt-2 sm:ps-5 sm:pt-0">
					<!-- <div class="text-lg font-semibold">Fostering African Entrepreneurship</div> -->
					<!-- <div class="text-sm text-muted-foreground">Helping</div> -->
				</div>
				<div class="mx-auto max-w-2xl rounded-lg p-3 sm:flex sm:space-x-3">
					<p>
						We're not just building websites – we're amplifying African innovation, one startup at a
						time. We believe, you're not just starting up; you're standing out in the digital world,
						representing the best of African entrepreneurship
					</p>
				</div>
				<Separator class="my-1" />

				<div class="flex w-full items-center justify-center">
					<Button href="##" class="mx-auto" size="lg">Create your own Site</Button>
				</div>
			</form>
			<!-- {/* End Form */} -->
			<!-- {/* SVG Element */} -->
			<div
				class="absolute start-0 top-2/4 hidden -translate-x-40 -translate-y-2/4 transform md:block lg:-translate-x-80"
				aria-hidden="true"
			>
				<svg
					class="h-auto w-52"
			...
			<!-- // {/* SVG Element */} -->
			<div
				class="absolute end-0 top-2/4 hidden -translate-y-2/4 translate-x-40 transform md:block lg:translate-x-80"
				aria-hidden="true"
			>
				<svg
					class="h-auto w-72"
